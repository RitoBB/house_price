...
if __name__ == '__main__':
    for i in range(0, 1929):
        importlib.reload(GibbsSamplingDMM)
        c = sys.argv[1:]
        c[1] = c[1] + str(i)
        model = GibbsSamplingDMM.GibbsSamplingDMM(check_arg(c))
        model.analyseCorpus()
        model.topicAssigmentInitialise()
        model.inference()

        print ("Writing Results",c)
        model.writeTopTopicalWords(i)
        model.writeTopicAssignments(i)
