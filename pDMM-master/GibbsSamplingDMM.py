...
class GibbsSamplingDMM(object):
	...
	def writeTopicAssignments(self,i):
		file = open(self.output+self.name+str(i)+".topicAssignments","w")
		#for i in range(self.numDocuments):
		[file.write(str(self.topicAssignments[i])+"\n") for i in range(self.numDocuments)]



	def writeTopTopicalWords(self,i):
		file = open(self.output+self.name+str(i)+".topWords","w")
		for t in range(self.ntopics):
